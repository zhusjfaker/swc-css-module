...
use swc_css_ast::{ComplexSelector, ComplexSelectorChildren, CompoundSelector, PseudoClassSelectorChildren, SelectorList, SubclassSelector, Token, TokenAndSpan};
...
impl CssModuleComponent {
  ...
  pub fn new(filepath: &str, content: &str) -> Self {
    CssModuleComponent {
      hash_perfix: StyleHash::generate_css_module_hash(filepath, content),
  ...
  pub fn child_select_compile(select_txt: &str) -> ComplexSelector {
    ...
    let mut visitor = ChildCssModuleComponent { selector_list: None };
    stylesheet.fold_with(&mut visitor);
    let selector_list = visitor.selector_list.unwrap();
    selector_list.children.get(0).unwrap().to_owned().into()
  }
  ...
  pub fn convert_vec_pseudoclasschildren(list: Vec<PseudoClassSelectorChildren>) -> ComplexSelector {
                ...
                match sub_ss {
                  ...
                  SubclassSelector::Class(mut class) => {
                    if !self.get_ident(index) {
                      ...
                      let mut class_selector_txt_value: String = class.text.value.to_string();
                      class_selector_txt_value = self.hash_perfix.clone() + "_" + class_selector_txt_value.as_str();
                      class.text.raw = JsWord::from(class_selector_txt_value.as_str());
                  ...
                  SubclassSelector::PseudoClass(prseudo_class) => {
                    if prseudo_class.name.value.to_string() == "global" {
                      if prseudo_class.children.is_none() {
                        ...
                        // example: -> :global(.x h2)
                        let mut child_stylesheet = Self::convert_vec_pseudoclasschildren(prseudo_class.children.unwrap());
                        complex_selector_children_list.append(&mut child_stylesheet.children);
                        break;
                      }
                      ...
                      if prseudo_class.children.is_none() {
                        ...
                        for child_complex_selector_children in child_stylesheet.children.iter_mut() {
                      ...
                      sub_list.push(SubclassSelector::PseudoClass(prseudo_class));
                    }
                  }
                }
              }
              complex_selector_children_list.push(ComplexSelectorChildren::CompoundSelector(CompoundSelector {
                subclass_selectors: sub_list,
                ..compound_selector
              }));
            } else {
              complex_selector_children_list.push(ComplexSelectorChildren::CompoundSelector(compound_selector));
            }
          }
